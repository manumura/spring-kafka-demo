...
public class QuicksortTest {
    ...
    static int partition(int[] arr, int l, int r) {
        System.out.println("partitioning with pivot " + arr[r] + " (at position " + r + ") in " + Arrays.toString(arr));
        int pivot = arr[r];
        int i = l - 1;
        for (int j = l; j < r; j++) {
            if (arr[j] < pivot) {
    ...
    static void swap(int[] arr, int i, int j) {
        System.out.println("swapping " + arr[i] + " (at position " + i + ") and " + arr[j] + " (at position " + j + ") in " + Arrays.toString(arr));
        int temp = arr[i];
        arr[i] = arr[j];
        arr[j] = temp;
    }
